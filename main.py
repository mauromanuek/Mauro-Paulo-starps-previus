...
def connect_ws(url, api_token):
    ...
    if auth_response.get('error'):
        # Mantemos o raise para que o ERRO NO CICLO seja apanhado e o bot tente novamente
...
def strategy_selection_engine(df, granularity_minutes):
    """
    Analisa o contexto e escolhe a estratégia. Prioridade: BBands (90%) > RSI (88%) > Stoch Rígido (85%) > Stoch Oportunista (80%).
    """
    ...
    if not np.isnan(bbands_upper) and current_close > bbands_upper:
        ...
        trend = "PUT"
        confidence = 90
        justification = f"REVERSÃO FORTE (90%). Preço ({current_close:.4f}) acima da Banda Superior de Bollinger ({bbands_upper:.4f})."
    ...
    if not np.isnan(bbands_lower) and current_close < bbands_lower:
        ...
        trend = "CALL"
        confidence = 90
        justification = f"REVERSÃO FORTE (90%). Preço ({current_close:.4f}) abaixo da Banda Inferior de Bollinger ({bbands_lower:.4f})."
        return trend, justification, confidence, indicator_status, strategy_used
    ...
    if not np.isnan(current_rsi) and current_rsi > 70 and conf_put:
        ...
        trend = "PUT"
        confidence = 88
        justification = f"REVERSÃO RÁPIDA (88%). RSI em SOBRECOMPRA (>70). CONFIRMAÇÃO: {conf_put}."
        return trend, justification, confidence, indicator_status, strategy_used
    ...
    if not np.isnan(current_rsi) and current_rsi < 30 and conf_call:
        ...
        trend = "CALL"
        confidence = 88
        justification = f"REVERSÃO RÁPIDA (88%). RSI em SOBREVENDA (<30). CONFIRMAÇÃO: {conf_call}."
        return trend, justification, confidence, indicator_status, strategy_used

    # --- 3. ESTRATÉGIA STOCHASTIC RÍGIDA (Confirmação de Sucesso - 85%) ---
    if not np.isnan(current_stoch_k):
        # Stoch > 80 E Confirmação de Resistência
        if current_stoch_k > 80 and conf_put: 
            strategy_used = "Stoch RÍGIDO (Reversão de Extremos com S/R)"
            trend = "PUT"
            confidence = 85
            justification = f"ALTA CONFIANÇA (85%). Stochastic em SOBRECOMPRA (>80). Confirmação de Sucesso: {conf_put}."
            return trend, justification, confidence, indicator_status, strategy_used
        ...
        # Stoch < 20 E Confirmação de Suporte
            ...
            trend = "CALL"
            confidence = 85
            justification = f"ALTA CONFIANÇA (85%). Stochastic em SOBREVENDA (<20). Confirmação de Sucesso: {conf_call}."
            return trend, justification, confidence, indicator_status, strategy_used

        # --- 4. ESTRATÉGIA STOCHASTIC OPORTUNISTA (Aproveitamento - 80%) ---
        # Só chega aqui se o sinal de 85% acima NÃO tiver sido ativado.
        if current_stoch_k > 80:
            strategy_used = "Stoch OPORTUNISTA (Reversão Pura)"
            trend = "PUT"
            confidence = 80
            justification = f"OPORTUNIDADE (80%). Stochastic em SOBRECOMPRA (>80). Sem Confirmação S/R, mas extremo é claro."
            return trend, justification, confidence, indicator_status, strategy_used
            ...
            trend = "CALL"
            confidence = 80
            justification = f"OPORTUNIDADE (80%). Stochastic em SOBREVENDA (<20). Sem Confirmação S/R, mas extremo é claro."
            return trend, justification, confidence, indicator_status, strategy_used
    ...
    # --- 5. ESTRATÉGIA DE CURTO PRAZO (EMA Breakout - 70%) ---
        ...
        if current_close > current_ema:
            ...
            trend = "CALL"
            confidence = 70
            justification = f"Preço acima da EMA(10). Não há extremos de alta confiança ativos."

        elif current_close < current_ema:
            strategy_used = "Acompanhamento de Curto Prazo (EMA Breakout)"
            trend = "PUT"
            confidence = 70
            justification = f"Preço abaixo da EMA(10). Não há extremos de alta confiança ativos."
            
    return trend, justification, confidence, indicator_status, strategy_used


# --- FUNÇÕES FETCH (Mantidas) ---
...
def monitor_ticks_and_signal(ws, symbol, trend, justification, confidence, indicator_status, strategy_used, granularity_minutes):
    """Monitoriza ticks e gera o Sinal Final (Timing) com limite de aproveitamento (30s)."""
    
    # --- NOVO: Janela de Aproveitamento ---
    APROVEITAMENTO_JANELA_SEGUNDOS = 30 
    start_time = time.time() # Tempo atual do sistema (identificação do sinal)
    ...
    try:
        while BOT_STATUS == "ON":
            # 1. VERIFICAÇÃO DO TEMPO DE APROVEITAMENTO
            if time.time() - start_time > APROVEITAMENTO_JANELA_SEGUNDOS:
                add_log(f"AVISO: Janela de aproveitamento de {APROVEITAMENTO_JANELA_SEGUNDOS}s expirada. Sinal {trend} ({confidence}%) ignorado.")
                
                # Zera o sinal se o tempo de aproveitamento esgotar
                update_signal_data({
                    'direction': 'EXPIRADO', 
                    'confidence': 0,
                    'justification': 'O sinal de alta confiança não foi aproveitado na janela de 30 segundos.'
                })
                break 
            
            # 2. PROCESSAMENTO NORMAL DE TICKS
            message = ws.recv()
            ...
            if data.get('tick'):
                ...
                # SUCESSO: O tick chegou dentro da Janela de Aproveitamento
                update_signal_data({
                    'direction': trend, 
            ...
            try:
                ...
                if trend != "NEUTRA":
                    if trend == macro_trend:
                        # Aumenta a confiança de QUALQUER sinal (70%, 80%, 85%, 88%, 90%)
                        ...
                        # Zera o sinal se for contra o macro (a não ser que seja 88%+ RSI/BBands)
                        if confidence < 88: 
                            final_trend = "NEUTRA"
                            final_confidence = 5
                ...
                # *** AJUSTE FINAL DE ROBUSTEZ APLICADO AQUI ***
                time.sleep(90) # Aumentado para 90s (1.5 min) para evitar erros de autenticação. 
